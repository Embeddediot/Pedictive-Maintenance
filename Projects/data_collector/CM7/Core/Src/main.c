...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  /* USER CODE BEGIN 2 */
<<<<<<< HEAD
  iis3dwb_init(); // Initialize the IIS3DWB sensor
...
<<<<<<< HEAD
	...
	{
		sprintf((char *)tx_buffer, "Temperature [degC]:%3.2f, Acceleration [mg]:%3.2f\r\n", temp_data, accel_data);
		printf("%s", tx_buffer);
	}
...
=======
  ...
  while(1)
  {
      ...
      if (dataFlag) {
        GPIOB->BSRR = GPIO_BSRR_BS0; // DEBUG
        dataFlag = 0;
